...
public class RelationAnalyzer extends DefaultTraversalVisitor<AnalyzedRelation, StatementAnalysisContext> {
    ...
    protected AnalyzedRelation visitQuerySpecification(QuerySpecification node, StatementAnalysisContext statementContext) {
        ...
        if (isDistinct) {
            ...
            if (Sets.newHashSet(selectAnalysis.outputSymbols()).containsAll(groupBy)) {
                distinctProcessed = true;
            }
        }
    ...
    private final static class FieldReplacer extends ReplacingSymbolVisitor<Void>
                                             implements java.util.function.Function<Symbol, Symbol> {

        private final QueriedRelation relation;

        FieldReplacer(QueriedRelation relation) {
        ...
        public Symbol visitField(Field field, Void context) {
            return new Field(relation, field.path(), field.valueType());
        }
